...
namespace BitBenderGames {
  ...
  public class TouchInputController : MonoBehaviour {
    ...
    [SerializeField]
    [Tooltip("A double click is recognized when the distance between two consecutive taps is smaller than this value in pixels.")]
    private float doubleclickDistanceThreshold = 300;
    ...
    public bool IsInputOnLockedArea {
      get { return isStopInput; }
      set { isStopInput = value; }
    }

    /// <summary>
    /// 是否停止输入
    /// 解决拖动冲突问题 sunshuo 2024/07/17
    /// </summary>
        ...
        if (isPinching == false && isDragging == false && wasPinchingLastFrame == false && wasDraggingLastFrame == false && isClickPrevented == false) {
          ...
          if (wasFingerDownLastFrame == true && TouchWrapper.IsFingerDown == false) {
            ...
            if (wasDraggingLastFrame == false && wasPinchingLastFrame == false) {
              ...
              bool isDoubleClick = clickDuration < doubleclickDurationThreshold && Vector2.Distance(lastFinger0DownPos, lastClickPosition) < doubleclickDistanceThreshold;
              bool isLongTap = fingerDownUpDuration > clickDurationThreshold;

              lastClickPosition = lastFinger0DownPos;
