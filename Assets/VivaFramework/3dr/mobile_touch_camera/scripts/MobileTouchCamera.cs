...
namespace BitBenderGames {
  ...
  public class MobileTouchCamera : MonoBehaviourWrapped {
    ...
    [SerializeField]
    [Tooltip("Default keeps the camera from looking beyond the boundary. Clamp just clamps the camera position to the boundary values.")]
    private BoundaryMode boundaryMode = BoundaryMode.DEFAULT;
    [SerializeField]
...
#pragma warning restore 649
    ...
    private Vector2 camOverdragMargin1dTo2d = Vector2.one;
    private Camera cam;

    private float timeRealDragStop;
    private bool isPinchCenterTooFar;
    private Vector3 lastValidPinchCenter;
    ...
    public LuaFunction updatePosCall;
    ...
    public Vector2 CamOverdragMargin2d {
      get {
        if (is2dOverdragMarginEnabled) {
          ...
          camOverdragMargin1dTo2d.x = camOverdragMargin;
          camOverdragMargin1dTo2d.y = camOverdragMargin;
          return camOverdragMargin1dTo2d;
        }
      }
      set {
    ...
    public bool Is2dOverdragMarginEnabled {
      get => is2dOverdragMarginEnabled;
      set => is2dOverdragMarginEnabled = value;
    }
...
#pragma warning disable 414
    [System.Obsolete("This variables is replaced by the boundaryMode variable that can be tweaked from the inspector.")]
    private bool useUntransformedCamBoundary = true;
#pragma warning restore 414
    private float maxHorizonFallbackDistance = 1000000; //This value may need to be tweaked when using the camera in a low-tilt, see above the horizon scenario.
    private bool useOldScrollDamp = true;
    private bool enableKeyboardZoomHotkeys = false; //Enable the hotkeys number 1-3 to zoom the camera.
    #endregion
    ...
    public Vector3 GetIntersectionPoint(Ray ray) {
      ...
      if (success == false || (Cam.orthographic == false && distance > maxHorizonFallbackDistance)) {
        ...
        if (showHorizonError == true) {
          string errorMessage = "Failed to compute intersection between camera ray and reference plane. Make sure the camera Axes are set up correctly.";
          if(BoundaryMode != BoundaryMode.CLAMP_POSITION) {
            errorMessage += " If you want to use the camera at low tilt angles, consider setting Boundary Mode to CLAMP_POSITION in the Advanced inspector settings of the Mobile Touch Camera component.";
          }
    ...
    public void UpdateTiltForAutoTilt(float newCameraSize) {
    ...
    private void DoPositionUpdateForTilt(bool isSpringBack) {
      ...
      if(isSpringBack == true || (isPinchTiltMode == true && isPinchModeExclusive == true)) {
        ...
        if(isPinchCenterTooFar == false) {
          intersectionDragCurrent = GetIntersectionPoint(Cam.ScreenPointToRay(pinchCenterCurrentLerp));
        } else {
          intersectionDragCurrent = GetIntersectionPoint(GetCamCenterRay());
        }
      }
    ...
    private float GetCurrentTiltAngleDeg(Vector3 rotationAxis) {
      ...
      float tiltAngle = Vector3.Angle(camForwardOnPlane, -Transform.forward);
      var isBelowPlane = Vector3.Dot(RefPlane.normal, -Transform.forward) < 0;
      if(isBelowPlane) {
        tiltAngle = -tiltAngle;
      }
      return(tiltAngle);
        ...
        if (Mathf.Approximately(mouseScrollDelta, 0) == false) {
          if (isEditorInputRotate == true) {
            if (EnableRotation == true) {
              Vector3 rotationAxis = GetRotationAxis();
              if(IsTooFar(mouseRotationCenter) == false) {
            ...
            DoManualZoom(CamZoom - zoomAmount);
          }
        }
        if(enableKeyboardZoomHotkeys) {
          for (int i = 0; i < 3; ++i) {
    ...
    private void DoManualZoom(float newZoom) {
      float camSizeDiff = DoEditorCameraZoom(CamZoom - newZoom);
      ...
      Vector3 pinchFocusVector = GetIntersectionPoint(Cam.ScreenPointToRay(TouchWrapper.LastAverageTouchPos)) - intersectionScreenCenter;
    ...
    private void InputControllerOnDragUpdate(Vector3 dragPosStart, Vector3 dragPosCurrent, Vector3 correctionOffset) {
      if (isDraggingSceneObject == false) {
        if (IsTooFar(dragPosCurrent + correctionOffset)) {
          Debug.LogWarning("Max Fallback Distance reached. Disable dragging.");
    ...
    private void ValidatePinchCenter(bool isTooFar, ref Vector3 pinchCenter) {
      if (isTooFar) {
        pinchCenter = lastValidPinchCenter;
      }
      lastValidPinchCenter = pinchCenter;
    }

    private void InputControllerOnPinchStart(Vector3 pinchCenter, float pinchDistance) {

      isPinchCenterTooFar = IsTooFar(pinchCenter);
      ValidatePinchCenter(isPinchCenterTooFar, ref pinchCenter);
      ...
      if(isPinchCenterTooFar == false) {
        ...
        Debug.LogWarning("Pinch center too far. Using cam center-ray.");
    ...
    private void InputControllerOnPinchUpdate(PinchUpdateData pinchUpdateData) {
      ...
      if(isPinchCenterTooFar == false) {
        isPinchCenterTooFar = IsTooFar(pinchUpdateData.pinchCenter);
      }
      ValidatePinchCenter(isPinchCenterTooFar, ref pinchUpdateData.pinchCenter);
    ...
    public IEnumerator ZoomToTargetValueCoroutine(float target, float duration = 0.3f) {
      ...
      if (Mathf.Approximately(target, CamZoom) == false) {
        ...
        while (Time.time < timeStart + duration) {
          ...
          var newCamZoom = Mathf.Lerp(startValue, target, Mathf.Sin(-Mathf.PI*0.5f + progress * Mathf.PI)*0.5f + 0.5f);
          DoManualZoom(newCamZoom);
          yield return null;
        }
        DoManualZoom(target);
      }
    ...
    private bool IsTooFar(Vector3 screenPoint) {
      float distance = 0;
      var ray = Cam.ScreenPointToRay(screenPoint);
      bool success = RefPlane.Raycast(ray, out distance);
      return success == false || (Cam.orthographic == false && distance > maxHorizonFallbackDistance);
    }
