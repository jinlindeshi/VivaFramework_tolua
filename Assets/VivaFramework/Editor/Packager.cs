...
//                Debug.Log("HandlerScenes2 - " + build.assetBundleName + " - " + fixedFiles.Count);
    ...
    static void HandleLuaBundle() {
        string streamDir = Application.dataPath + "/" + AppConst.LuaTempDir;
//        UnityEngine.Debug.LogError("HandleLuaBundle " + Application.dataPath);
        if (!Directory.Exists(streamDir)) Directory.CreateDirectory(streamDir);

        string[] srcDirs = { CustomSettings.luaDir};
//        UnityEngine.Debug.LogError("HandleLuaBundle1" + srcDirs.Length);
...
//				UnityEngine.Debug.LogError("HandleLuaBundle2" + srcDirs[i]);
				ToLuaMenu.CopyLuaBytesFiles(srcDirs[i], streamDir);
            }
        }
        AddBuildMapAll("lua/lua" + AppConst.ExtName, "Assets/" + AppConst.LuaTempDir);
        ...
        for (int i = 0; i < srcDirs.Length; i++) {
            ...
            foreach (string f in files) {
				if (f.EndsWith(".meta") || f.EndsWith(".lua") || f.Contains(".idea")) continue;
